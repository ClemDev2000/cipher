...
get_latest() {
    ...
    if [ -z "$GITHUB_PAT" ]; then
        curl -s 'https://api.github.com/repos/ClemDev2000/cipher/releases' > "$temp_file" || return 1
    else
        curl -H "Authorization: token $GITHUB_PAT" -s 'https://api.github.com/repos/ClemDev2000/cipher/releases' > "$temp_file" || return 1
    fi
...
success_usage() {
    printf "$GREEN%s\n$DEFAULT" "Cipher $latest binary successfully downloaded as '$binary_name' file."
    echo ''
    echo 'Run it:'
    echo '    $ ./cipher'
    echo 'Usage:'
...
not_available_failure_usage() {
    printf "$RED%s\n$DEFAULT" 'ERROR: Cipher binary is not available for your OS distribution or your architecture yet.'
    echo ''
...
fetch_release_failure_usage() {
    echo ''
    printf "$RED%s\n$DEFAULT" 'ERROR: Impossible to get the latest stable version of Cipher.'
    echo 'Please let us know about this issue: https://github.com/ClemDev2000/cipher/issues/new/choose'
}
...
echo "Downloading Cipher binary $latest for $os, architecture $archi..."
case "$os" in
    'windows')
        release_file="cipher-$os-$archi.exe"
        binary_name='cipher.exe'

        ;;
    *)
        release_file="cipher-$os-$archi"
        binary_name='cipher'

esac

# Fetch the Cipher binary
link="https://github.com/ClemDev2000/cipher/releases/download/$latest/$release_file"
curl --fail -OL "$link"
if [ $? -ne 0 ]; then
    fetch_release_failure_usage
    exit 1
